...
class ToTensor:
    def __call__(self, image, bboxes, labels):
        if isinstance(image, tuple):
            return tuple([self._to_tensor(image) for image in image]), bboxes, labels
        else:
            return self._to_tensor(image), bboxes, labels
...
class ToNumpy:
    def __call__(self, PIL_image, bboxes=None, labels=None):
        return np.asarray(PIL_image, dtype='uint8'), bboxes, labels
